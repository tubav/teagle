...
class SimpleSoapClient(object):
    ...
    def __init__(self,host,path,descriptor,scheme="http", port = None, certfile = None, keyfile = None):
        '''
        @param remote host
        @param remote path
        ...
        @param remote scheme
        '''
        self.port = port or getservbyname(scheme)
        self.host = host
        self.path = path
    ...
    def __call__(self,*args,**kwargs):
        ...
        if self.scheme == "http": 
            ...
            conn = httplib.HTTPSConnection(self.host, self.port, self.keyfile, self.certfile)
        else:
            raise RuntimeError("Unsupported URI connection scheme: %s" % self.scheme)
...
class ServiceClient(object):
    ...
    def __init__(self,host,path,server_impl, scheme="http", certfile = None, keyfile = None):
        ...
        for method in self.server.methods():
            setattr(self,method.name,SimpleSoapClient(host,path,method,scheme, port = port, certfile = certfile, keyfile = keyfile))

def make_service_client(url,impl, certfile = None, keyfile = None):
    scheme,host,path = split_url(url)
    return ServiceClient(host,path,impl, scheme, certfile = certfile, keyfile = keyfile)
