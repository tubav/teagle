...
import logging
import time
logger = logging.getLogger("VmAdapter")
...
class VmAdapter(AbstractResourceAdapter):
    ...
    def __init__(self, manager, *args, **kw):
        ...
        self.__manager = manager.register_adapter("/vmadapter", self)
    ...
    def list_resources(self, parent, typename):
        return [ Identifier("/vmadapter-" + i) for i in self.__instances ]

    def add_resource(self, parent_id, name, typename, config, owner = None):
        assert(typename == "vmadapter")
	...
	self.__poll_machine()
        ...
        self.__pending_vms['/vmadapter-%s' %name] = vmConf
    ...
    def have_resource(self, identifier):
        ...
        assert(identifier.typename == "vmadapter")
