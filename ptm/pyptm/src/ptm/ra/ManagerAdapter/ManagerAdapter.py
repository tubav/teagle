...
class PythonResourceAdapter(Resource):
    def __init__(self, config, *args, **kw):
        super(PythonResourceAdapter, self).__init__(type = "pythonresourceadapter", *args, **kw)
...
class ManagerAdapter(AbsoluteParentMixin, MangleConfigMixin, ResourceAdapter):
    ...
    def _get_resource(self, uuid):
        if uuid.typename == "pyromanager":
            ...
            return PyroManager(identifier = uuid, adapter = self)
    ...
    def _list_resources(self, parentId, typename):
        if parentId == self.parent_id:
            ...
            return ( PyroManager(adapter = self, parent = parentId), )
        ...
        with self.__lock:
            return [ PythonResourceAdapter(adapter = self, parent = parentId, name = name, config = config) for name, config in self.__adapters.iteritems() if name != "order" ]
