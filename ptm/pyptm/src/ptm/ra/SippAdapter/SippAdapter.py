...
import logging
import paramiko
logger = logging.getLogger("SippAdapter")
'''
Iperf Adapter for demo, to be used only on a fedora machine
'''
class SippAdapter(AbstractResourceAdapter):
    '''
    classdocs
    '''
    ...
    def __init__(self, manager, *args, **kw):
        super(SippAdapter, self).__init__(*args, **kw)
        self.__instances = set("0")
        self.__manager = manager
        self.__manager = manager.register_adapter("/sippadapter", self)
        self.__install_command_fed = 'yum install -y sipp screen'
        self.__install_command_deb = 'apt-get install -y sip-tester screen'
        logger.debug("---up---")
        
    def list_resources(self, parent, typename):
        return [ Identifier("/sippadapter-" + i) for i in self.__instances ]

    def add_resource(self, parent_id, name, typename, config, owner = None):
        assert(typename == "sippadapter")
        install = False
        sipp_type = 'server'
        ...
        if config.has_key("install"):
            install = True
        if config.has_key("sipp_type"):
            sipp_type = config['sipp_type']
        ...
        if(sipp_type == 'server'):
            start_command = 'screen -m -d -S sipp /bin/bash -c "/usr/bin/sipp -sn uas"'
        else:
            start_command = 'screen -m -d -S sipp /bin/bash -c "/usr/bin/sipp -sn uac %s"'%server_address
    ...
    def have_resource(self, identifier):
        ...
        assert(identifier.typename == "sippadapter")
