...
#For this (very simple) example we will implement a basic RA that manages only onetype of resource: called "sgw".
...
class Sgw(AbstractResourceAdapter):
    '''
    classdocs
    '''
    ...
    def __init__(self, parent, manager, *args, **kw):
        super(Sgw, self).__init__(*args, **kw)
        
        #this is where we store our instances. We simply store the names of all existing instances.
        #here, one instance ("0") already exists. The full id of this instance would be "/sgw-0"
        self.__instances = set("0")
        
        #tell the manager to announce that we are responsible for that type "sgw" at the root of the hierarchy
        manager.register_adapter("/sgw", self)
        manager.register_adapter("/switch*/sgw", self)
    ...
    def list_resources(self, parent, typename):
        ...
        #since we have only registered for the type "sgw", thats the only type that can be requested
        assert(typename == "sgw" or not typename)
...
#        assert(parent == None)
        ...
        return [ Identifier("/sgw-" + i) for i in self.__instances ]
    ...
    def add_resource(self, parent_id, name, typename, config, owner = None):
        ...
        #we are lazy and do the latter. The Framework will puzzle "/sgw-<name>" together from it
        return name

    #tell the system if a given id exists
    def have_resource(self, identifier):
#        assert(identifier.parent == None)
        assert(identifier.typename == "sgw")
