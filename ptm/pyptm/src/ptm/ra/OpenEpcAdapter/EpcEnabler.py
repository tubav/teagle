...
logger = logging.getLogger("EpcEnabler")

#For this (very simple) example we will implement a basic RA that manages only onetype of resource: called "epcenabler".
...
class EpcEnabler(AbstractResourceAdapter):
    '''
    classdocs
    '''
    ...
    def __init__(self, parent, manager, *args, **kw):
        super(EpcEnabler, self).__init__(*args, **kw)
        
        #this is where we store our instances. We simply store the names of all existing instances.
        #here, one instance ("0") already exists. The full id of this instance would be "/epcenabler-0"
        self.__instances = set("0")
        self.__command = "/opt/OpenEPC/sbin/f4firedemo_start.sh"
        #tell the manager to announce that we are responsible for that type "epcenabler" at the root of the hierarchy
        manager.register_adapter("/epcenabler", self)
        manager.register_adapter("/switch*/epcenabler", self)
    ...
    def list_resources(self, parent, typename):
        ...
        #since we have only registered for the type "epcenabler", thats the only type that can be requested
        assert(typename == "epcenabler" or not typename)
...
#        assert(parent == None)
        ...
        return [ Identifier("/epcenabler-" + i) for i in self.__instances ]
    ...
    def add_resource(self, parent_id, name, typename, config, owner = None):
        ...
        #we are lazy and do the latter. The Framework will puzzle "/epcenabler-<name>" together from it
        return name

    #tell the system if a given id exists
    def have_resource(self, identifier):
#        assert(identifier.parent == None)
        assert(identifier.typename == "epcenabler")
...
#        assert(identifier.parent == None)
    ...
    def execute_start(self, identifier, **kw):
        ...
        subprocess.Popen(["/opt/OpenEPC/sbin/f4firedemo_start.sh"]).wait()
	return
    
    def execute_stop(self, identifier, **kw):
        logger.debug("stop method")
        subprocess.Popen(["/opt/OpenEPC/sbin/f4firedemo_stop.sh"]).wait()
	return		 
