...
#For this (very simple) example we will implement a basic RA that manages only onetype of resource: called "switch".
...
class Switch(AbstractResourceAdapter):
    '''
    classdocs
    '''
    ...
    def __init__(self, parent, manager, *args, **kw):
        ...
        #here, one instance ("0") already exists. The full id of this instance would be "/switch-0"
        self.__instances = set("0")
        
        #tell the manager to announce that we are responsible for that type "switch" at the root of the hierarchy
        manager.register_adapter("/switch", self)
    ...
    def list_resources(self, parent, typename):
        ...
        #since we have only registered for the type "switch", thats the only type that can be requested
        assert(typename == "switch" or not typename)
        ...
        return [ Identifier("/switch-" + i) for i in self.__instances ]
    ...
    def add_resource(self, parent_id, name, typename, config, owner = None):
        ...
        #we are lazy and do the latter. The Framework will puzzle "/switch-<name>" together from it
        return name

    #tell the system if a given id exists
    def have_resource(self, identifier):
        assert(identifier.parent == None)
        assert(identifier.typename == "switch")
