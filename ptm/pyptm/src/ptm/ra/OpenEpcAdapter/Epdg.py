...
#For this (very simple) example we will implement a basic RA that manages only onetype of resource: called "epdg".
...
class Epdg(AbstractResourceAdapter):
    '''
    classdocs
    '''
    ...
    def __init__(self, parent, manager, *args, **kw):
        super(Epdg, self).__init__(*args, **kw)
        
        #this is where we store our instances. We simply store the names of all existing instances.
        #here, one instance ("0") already exists. The full id of this instance would be "/epdg-0"
        self.__instances = set("0")
        
        #tell the manager to announce that we are responsible for that type "epdg" at the root of the hierarchy
        manager.register_adapter("/epdg",self)
        manager.register_adapter("/switch*/epdg",self)
    ...
    def list_resources(self, parent, typename):
        ...
        #since we have only registered for the type "epdg", thats the only type that can be requested
        assert(typename == "epdg" or not typename)
...
#        assert(parent == None)
        ...
        return [ Identifier("/epdg-" + i) for i in self.__instances ]
    ...
    def add_resource(self, parent_id, name, typename, config, owner = None):
        ...
        #we are lazy and do the latter. The Framework will puzzle "/epdg-<name>" together from it
        return name

    #tell the system if a given id exists
    def have_resource(self, identifier):
#        assert(identifier.parent == None)
        assert(identifier.typename == "epdg")
