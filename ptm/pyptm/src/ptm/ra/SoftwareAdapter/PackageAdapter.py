...
class PackageAdapter(ReflectiveConstructorAddAdapter, ReflectiveGetAdapter, ReflectiveListAdapter):
	def __init__(self, manager, parent, engine = None, plugindir = None, repodir = None, baseclass = Package, paranoid = False, *args, **kw):
		...
		if engine is None:
			engine = "sqlite:///" + (self.get_homedir() / "swa.db")

		logger.debug("Using database engine: %s", engine)
	...
	def __get_reference(self, o, k):
		...
		identifier = getattr(o, k)
		return self.__unpack_reference(identifier)

	def __unpack_reference(self, identifier):
	...
	def _add_resource(self, parent, name, typename, config):
		...
		try:
			self._deploy_resource(e, session)
		except:
	...
	def _deploy_resource(self, e, session):
		e.deploy()
		try:
			e.postdeploy()
		except:
...
class SoftwareAdapter(PackageAdapter):
	...
	def _deploy_resource(self, e, session):
		...
		super(SoftwareAdapter, self)._deploy_resource(e, session)
	...
	def need_port(self, port, type):
		...
		return self.client.add_resource(Identifier(self.parent.identifier) / "ipv4interface-ANY", None, type + "port", dict(number = port))
		#return self.client.add_resource(Identifier(self.parent.identifier) / "ipv4interface-ANY" + "/" + type + "port", None, type + "port", dict(number = port))
